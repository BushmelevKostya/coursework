...
public class GenreService {
    ...
    public Page<GenreResponseDTO> getAllGenres(Pageable pageable) {
        return genreRepository.findAll(pageable).map(this::getDTOFromEntity);
    }


    public GenreResponseDTO getGenreById(Long id) {
        return genreRepository.findById(id).map(this::getDTOFromEntity)
    ...
    public GenreResponseDTO createGenre(GenreMutationDTO genreMutationDTO) {
        Genre genre = getEntityFromDTO(genreMutationDTO);
        genre = genreRepository.save(genre);

        return getDTOFromEntity(genre);
    }
    ...
    public GenreResponseDTO updateGenre(Long id, GenreMutationDTO genreMutationDTO) {
        ...
        return getDTOFromEntity(updatedGenre);
    }


    //TODO подумать над логикой удаления

    private GenreResponseDTO getDTOFromEntity(Genre genre) {
        return new GenreResponseDTO(genre.getId(), genre.getName());
    }

    private Genre getEntityFromDTO(GenreMutationDTO genreMutationDTO) {
